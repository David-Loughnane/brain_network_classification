...
for subject in range(1):#range(len(subjectIDs)):
	...
	plt.imshow(func_corr,interpolation='nearest')
	plt.colorbar()
	plt.title("Functional Correlation")
	plt.xlabel('Parcels')
	plt.ylabel('Parcels')
	fig03 = drawmatrix_channels(func_corr, size=[156., 156.], color_anchor=0)
	#plt.savefig('../plots/Functional {}.png'.format(subjectIDs[subject]))
'''
